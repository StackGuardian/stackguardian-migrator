...
SG_API_HINTS=(
  ...
  'integration.*(not found|does not exist|invalid)|(not found|does not exist|invalid).*integration => Connector/integration id not found in this org. Check SGDefaultVCSAuthIntegrationID and SGDefaultDeploymentPlatformConfig[].config.integrationId (or the projectOverrides / workspaceOverrides equivalents); '"'"'preflight'"'"' lists the ids that exist.'
  'runner ?group.*(not found|does not exist|invalid)|(not found|does not exist|invalid).*runner => Runner group not found. Check SGDefaultRunnerConstraints.names / projectOverrides[<project>].RunnerConstraints / workspaceOverrides[<ws>].RunnerConstraints against the runner groups in the SG org.'
  ...
  'approver => Approvers must be e-mail addresses of existing SG users. Check SGDefaultWfApprovers / projectOverrides[<project>].Approvers / workspaceOverrides[<ws>].Approvers.'
  'already exists|not unique => A resource with that name already exists. Existing workflows are updated by the importer (PATCH); an existing workflow group is reused.'
