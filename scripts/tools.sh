...
sg_retry() {
  ...
  while :; do
    ...
    if [ "$attempt" -ge "$max" ]; then
      sg_err "failed after ${attempt} attempt(s) (exit ${rc}): $*"
      return "$rc"
    fi
...
sg_resolve() {
  ...
  if [ -n "$ensure" ]; then "$ensure"; return $?; fi
...
sg_download() {
  local url="$1" dest="$2"
  command -v curl >/dev/null 2>&1 || { echo "curl is required but not found" >&2; return 1; }
...
sg_ensure_jq() {
  ...
  if [ ! -x "$bin" ]; then
    ...
    case "$(uname -s)" in Darwin) os="macos" ;; Linux) os="linux" ;; *) echo "Unsupported OS: $(uname -s)" >&2; return 1 ;; esac
...
sg_ensure_hcl2json() {
  ...
  if [ ! -x "$bin" ]; then
    arch=$(sg_arch) || return 1
    case "$(uname -s)" in Darwin) os="darwin" ;; Linux) os="linux" ;; *) echo "Unsupported OS: $(uname -s)" >&2; return 1 ;; esac
...
sg_ensure_sgcli() {
  ...
  if [ ! -x "$bin" ]; then
    case "$(uname -s)" in Darwin) os="Darwin" ;; Linux) os="Linux" ;; *) echo "Unsupported OS: $(uname -s)" >&2; return 1 ;; esac
    case "$(uname -m)" in x86_64 | amd64) arch="x86_64" ;; aarch64 | arm64) arch="arm64" ;; *) echo "Unsupported architecture: $(uname -m)" >&2; return 1 ;; esac
    ...
    if ! curl -fsSL "https://github.com/StackGuardian/sg-cli/releases/latest/download/sg-cli_${os}_${arch}.tar.gz" -o "$tmp/sg-cli.tar.gz"; then
      echo "Failed to download sg-cli" >&2; rm -rf "$tmp"; return 1
    fi
    tar -xzf "$tmp/sg-cli.tar.gz" -C "$tmp"
    realcli=$(find "$tmp" -maxdepth 2 -type f -name sg-cli | head -1)
    [ -n "$realcli" ] || { echo "sg-cli binary not found in release archive" >&2; rm -rf "$tmp"; return 1; }
