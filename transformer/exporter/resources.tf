resource "local_file" "data" {
  content  = local.data
  filename = "${path.module}/../../export/sg-payload.json"
}
...
resource "local_file" "generateTempTfFiles" {
  ...
  filename = "${path.module}/../../export/tfDir/${each.key}/main.tf"
}
...
resource "null_resource" "exportStateFiles" {
  ...
  provisioner "local-exec" {
    ...
    working_dir = "${path.module}/../../export/tfDir/${each.key}"
  }
}
...
resource "null_resource" "deleteTempTfFiles" {
  ...
  provisioner "local-exec" {
    command     = "rm -rf tfDir"
    working_dir = "${path.module}/../../export/"
  }
}